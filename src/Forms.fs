...
let getFieldValue (id: FieldId) (model: Model<_>): FieldState = 
    ...
    let field = model.Fields |> Field.Group
...
let private replaceListNode (id: FieldId) (model: Model<_>) replacer: Model<_> =
    let path = Path.parse id
    let rec set (path: Path.PathSegment list) (fields: Field option) =
        ...
        | (Path.List i ::rest, Some (List (l, d))) ->
            let newList = List.mapi (fun index (node: Group) ->
                    if index = i then
                        let (Field.Group g) = set rest (Some (Field.Group node))
                        g
                    else
                        node) l
            List <| (newList, d)
...
let setFieldValue (id: FieldId) (model: Model<_>) (fieldState: FieldState): Model<_> =
    let path = Path.parse id
    let rec set (path: Path.PathSegment list) (fields: Field option) (field: FieldState) =
        match (path, fields) with
        ...
        | (Path.List i ::rest, Some (List (l, d))) ->
            let updateListItem index item =
                if index = i then
                        let (Field.Group g) = set rest (Some (Field.Group item)) field
                        g
                    else
                        item
...
module Validator =
    ...
    and ValidatorT<'T> =
        {
            F: (Context -> Group -> ValidationResult<'T>)
            Default:  Field
            Context: Context
        }
    and Context =
    ...
    let from (f: 'T): Validator<'T> =
        Validator { F = (fun _ _ -> Ok f); Default = Form.Group Map.empty; Context = { Label = ""; Id = "" } }
    ...
    let apply (vf: Validator<_>) (va: Validator<_>): Validator<_> =
        ...
        Validator { F = inner; Default = Form.Group joinedGroups; Context = { Label = ""; Id = "" } }
    ...
    let withList id (validator: Validator<'a>): Validator<'a list> =
        let (Validator { F = v; Default = Group d; Context = c }) = validator
        let inner _ g =
            match Map.tryFind id g with
            | Some (List (l, _)) ->
                let mapper index g =
            ...
            | None ->
                Ok []
            | _ -> Error [ (id, "Invalid group type")  ]
        Validator { F = inner; Default = Form.Group <| Map.ofList [ id, List ([], d) ]; Context = { Label = ""; Id = id } }
    ...
    let text id =
        let inner _ (f: Group) =
            ...
            | None -> Ok None
            | _ -> Error [ (id, "Invalid group type")  ]
        Validator { F = inner; Default = Form.Group <| Map.ofList [ id, Leaf "" ]; Context = { Label = id; Id = id } }

    let required (f: Validator<'a option>) =
        let (Validator vv) = f
        let { F = v; Default = d } = vv
        let inner context (g: Group) =
    ...
    let run (validator: Validator<'T>) (form: Model<'T>) =
